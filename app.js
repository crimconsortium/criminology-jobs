(function () {
  ...
  function renderCoverage() {
    var html =
      '<ul>' +
      '<li><strong>ACJS (careers.acjs.org)</strong> — both public pages of the careers board collected on May 25, 2026. After applying the criminology / criminal-justice relevance filter, 3 specific postings retained; the rest excluded as adjunct-pool catch-alls, EMT / paramedic / paralegal, generic auxiliary-services roles, or unrelated counseling postings. Posted dates were given as relative ("X days ago") and converted to absolute dates relative to capture date.</li>' +
      '<li><strong>ASC (asc41.org)</strong> — single static page, no pagination, captured May 25, 2026. All 9 publicly listed postings captured. Two had deadlines that may have just lapsed (UNC Charlotte April 7; Illinois April 27) but were still listed at capture time.</li>' +
      '<li><strong>HigherEdJobs (higheredjobs.com)</strong> — Criminal Justice & Criminology faculty category, first 50 active postings captured (May 25, 2026). 17 retained as faculty + research roles (assistant/associate/full professors, lecturers, research professors, clinical faculty, department chairs); the rest excluded as adjunct, part-time, instructor pools, dual-credit, police-academy training, or non-criminology forensic-science/homeland-security postings. Two University of Idaho clinical-faculty posts cross-listed with ASC are merged into single rows.</li>' +
      '<li><strong>jobs.ac.uk</strong> — the keyword=criminology search captured May 25, 2026; 4 retained as clearly criminology-relevant (including criminology-adjacent sociology and law/social-justice posts where criminology is an explicit qualifying field). 8 excluded as pure law-school lectureships, generic LSE methodology fellowships, or unrelated technician/editor roles.</li>' +
      '<li><strong>TSPA (tspa.org)</strong> — the public Trust &amp; Safety job board, first indexed May 12, 2026. All 103 publicly listed postings carried over from the previous monthly TSPA refresh — TSPA covers industry trust-and-safety, policy, and content-moderation roles at tech companies (e.g. Google, Discord, Meta, TikTok). No filtering applied: roles span analyst, specialist, manager, lead, engineer, and director levels. Deadlines are not exposed on the source listing, so that column is blank for every TSPA row. <strong>Posted dates</strong> are also not exposed by TSPA; as a substitute, each TSPA row is stamped with the date this dashboard first indexed it, so "Newest posted" sort approximates "most recently seen" rather than the true publication date.</li>' +
      ...
      '<li><strong>De-duplication</strong> — records are matched across sources by normalized institution + normalized title; matches are merged into a single row with a comma-separated source_site (e.g. "ASC, HigherEdJobs") and all source URLs concatenated in combined_urls. The current snapshot has 3 cross-site duplicates merged into single rows.</li>' +
      "</ul>";
